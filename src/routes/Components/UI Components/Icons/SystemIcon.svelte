<script>
    export let size = 6; // Set the default size
    import { activemenu } from "$lib/CompanyTab/selectednavigationmenuitem";
    let colortint="";
    $: colortint = $activemenu === 4 ? "text-sky-500" : "";

</script>

<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 128 128" fill="none" stroke-width="1.5" stroke="currentColor" class="size-6 {colortint}">
    <path d="M110.8 87.514v-55.86a1.75 1.75 0 0 0-1.75-1.75h-90.1a1.75 1.75 0 0 0-1.75 1.75v55.86a1.75 1.75 0 0 0 1.75 1.75h90.1a1.75 1.75 0 0 0 1.75-1.75zm-3.5-1.75H20.7V33.4h86.6z"/>
    <path d="M121.693 92.487h-4.168V29.434a6.258 6.258 0 0 0-6.252-6.252H16.727a6.258 6.258 0 0 0-6.252 6.252v63.053H6.307a1.751 1.751 0 0 0-1.75 1.75v4.331a6.257 6.257 0 0 0 6.25 6.25h106.386a6.256 6.256 0 0 0 6.25-6.25v-4.331a1.75 1.75 0 0 0-1.75-1.75zM13.975 29.434a2.754 2.754 0 0 1 2.752-2.752h94.546a2.754 2.754 0 0 1 2.752 2.752v63.053H77.144a1.75 1.75 0 0 0-1.75 1.75v.935H52.606v-.935a1.751 1.751 0 0 0-1.75-1.75H13.975zm105.968 69.134a2.753 2.753 0 0 1-2.75 2.75H10.807a2.754 2.754 0 0 1-2.75-2.75v-2.581h41.049v.935a1.75 1.75 0 0 0 1.75 1.75h26.288a1.75 1.75 0 0 0 1.75-1.75v-.935h41.049z"/>
  </svg>